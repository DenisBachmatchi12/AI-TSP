...
class GeneticAlgorithm:
    ...
    def ruleaza_generatii(self, numar_generatii, sansa_mutatie):
        ...
        fitness_pe_generatie = []
        medie_fitness_pe_generatie = []
        ...
        for i in range(numar_generatii):
            ...
            # Calculează cel mai bun fitness și media fitness-ului din generație
            cel_mai_bun = min(generatie_curenta, key=lambda x: x[1])
            fitness_pe_generatie.append(cel_mai_bun[1])

            media_fitness = sum(individ[1] for individ in generatie_curenta) / len(generatie_curenta)
            medie_fitness_pe_generatie.append(media_fitness)
        ...
        # Plot fitness
        plt.plot(fitness_pe_generatie, marker='o', label="Fitness cel mai bun individ")
        plt.plot(medie_fitness_pe_generatie, marker='x', label="Media fitness populație")
        plt.xlabel("Generații")
        plt.ylabel("Fitness")
        plt.title("Evoluția fitness-ului în timp")
        plt.legend()
        plt.grid()
